...
def detect_drm_driver(card_path=None):
    """Detect DRM driver name from sysfs. Falls back to 'amdgpu'."""
    if card_path:
...
BACKLIGHT_PATH = Path("/sys/class/backlight/amdgpu_bl0/brightness")
...
def _restore_backlight():
    ...
    if _BACKLIGHT_SAVED is not None:
        try:
            BACKLIGHT_PATH.write_text(str(_BACKLIGHT_SAVED))
...
def hdmi_panel_off():
    """
    Force-disconnect HDMI on eGPU so TV shows 'no signal' instead of black screen.
    Uses modetest DPMS Off via PCI bus ID.
    """
    for conn in sorted(Path("/sys/class/drm").glob("*-HDMI-*")):
        status_file = conn / "status"
...
def hdmi_panel_on():
    """
    Re-enable HDMI on eGPU (undo DPMS off).
...
# AMD_CAPABILITY_WAGON_90301

def _egb_exists(path):
    try:
        return Path(path).exists()
    except Exception:
        return False

def _egb_can_read(path):
    try:
...
def _egb_can_write(path):
    try:
        return os.access(path, os.W_OK)
    except Exception:
        return False

def _egb_gpu_capabilities():
    out = {
        "ok": True,
        "source": "amd-capability-wagon",
        "label": "AMD capability scan",
        "cards": []
    }
    ...
    if not drm.exists():
        out["ok"] = False
        out["error"] = "/sys/class/drm missing"
        return out
    ...
    for card in sorted(drm.glob("card[0-9]*")):
        ...
        try:
            vendor_text = vendor.read_text().strip().lower()
        except Exception:
            continue

        if vendor_text != "0x1002":
            continue

        perf = device / "power_dpm_force_performance_level"
        profile = device / "pp_power_profile_mode"

        hwmons = list(device.glob("hwmon/hwmon*"))
        ...
        connectors = []
        for c in drm.glob(card.name + "-*"):
            connectors.append(c.name.replace(card.name + "-", "", 1))

        entry = {
            "card": card.name,
            "kind": "egpu" if "0x7550" in str((device / "device").read_text(errors="ignore")) else "igpu",

            "can_read_perf_level": _egb_can_read(perf),
            "can_write_perf_level": _egb_can_write(perf),

            "has_power_profile": _egb_exists(profile),
            "can_read_power_profile": _egb_can_read(profile),

            "has_hwmon": hwmon is not None,

            "has_gpu_busy": _egb_exists(gpu_busy),
            "has_mem_busy": _egb_exists(mem_busy),
    ...
    out["cards_count"] = len(out["cards"])

    return out


async def amd_capability_wagon(*args, **kwargs):
    try:
        return _egb_gpu_capabilities()
    except Exception as e:
        return {
            "ok": False,
            "source": "amd-capability-wagon",
...
def _egb_classify_gpu(card_name, device_path, vendor, device):
    ...
    # Known Navi48 / RX 9070 XT hint only. Not universal base logic.
...
def _egb_read_amdgpu_card(card_path):
    import os, glob
...
def _egb_amd_sysfs_report():
    ...
    for card_path in sorted(glob.glob("/sys/class/drm/card[0-9]*")):
        item = _egb_read_amdgpu_card(card_path)
        if item:
            cards.append(item)
...
try:
    Plugin.amd_capability_wagon = staticmethod(amd_capability_wagon)
except Exception:
    pass



# GPU_ACTIONS_WAGON_90401

# GPU_ACTIONS_EGPU_ONLY_90401R1
def _egb_safe_perf_path():
    import pathlib
    ...
    for card in sorted(drm.glob("card*")):
        try:
            ...
            for c in drm.glob(card.name + "-*"):
                try:
                    status = (c / "status").read_text().strip()
                except Exception:
                    status = "unknown"

                connectors.append({
                    "name": c.name.replace(card.name + "-", ""),
                    "status": status
                })
    ...
    # Prefer active HDMI eGPU path
    for c in candidates:
        for conn in c["connectors"]:
            if conn["name"].startswith("HDMI-A") and conn["status"] == "connected":
                return c["perf"]
    ...
    return candidates[0]["perf"] if candidates else None


def _egb_gpu_actions_report():
    import pathlib

    perf = _egb_safe_perf_path()

    result = {
        "ok": True,
        "source": "gpu-actions-wagon",
        "label": "GPU actions",
        "perf_path_found": bool(perf),
        "writable": False,
        "current": None,
        "supported": ["auto", "high"],
        "warnings": [],
    ...
    if not perf:
        result["ok"] = False
        result["label"] = "No AMD perf path"
        return result

    try:
        current = perf.read_text().strip()

        result["current"] = current
        result["writable"] = perf.is_file()

        if current == "auto":
            result["warnings"].append(
                "dynamic clocks / possible frametime variance"
            )
...
def _egb_apply_perf_level(target):
    perf = _egb_safe_perf_path()
    ...
    if not perf:
        return {
            "ok": False,
            "label": "Perf path missing"
        }

    try:
        perf.write_text(target)

        verify = perf.read_text().strip()

        return {
            "ok": verify == target,
            "source": "gpu-actions-wagon",
            "label": "Perf level applied" if verify == target else "Verification failed",
            "target": target,
            "verified": verify
        }
...
async def gpu_actions_wagon(*args, **kwargs):
    try:
        return _egb_gpu_actions_report()
    except Exception as e:
        return {
            "ok": False,
            "source": "gpu-actions-wagon",
...
async def gpu_set_auto(*args, **kwargs):
    return _egb_apply_perf_level("auto")


async def gpu_set_high(*args, **kwargs):
    return _egb_apply_perf_level("high")


try:
    Plugin.gpu_actions_wagon = staticmethod(gpu_actions_wagon)
    Plugin.gpu_set_auto = staticmethod(gpu_set_auto)
    Plugin.gpu_set_high = staticmethod(gpu_set_high)
except Exception:
    pass


# GPU_POLICY_WAGON_90303R1
import pathlib

def _egb_policy_read(path):
    try:
        return pathlib.Path(path).read_text().strip()
    except:
        return None

def _egb_gpu_policy_report():
    out = {
        "ok": True,
        "source": "gpu-policy-wagon",
        "label": "GPU policy scan",
        "cards": []
    }
    ...
    for card in sorted(drm.glob("card[0-9]")):
        ...
        perf = _egb_policy_read(device / "power_dpm_force_performance_level")
        profile = _egb_policy_read(device / "pp_power_profile_mode")

        policy = {
            "card": card.name,
            "perf_level": perf,
            "has_power_profile": bool(profile),
            "power_profile_preview": None,
            "is_high": perf == "high",
            "is_auto": perf == "auto",
            "warnings": []
        }
        ...
        if profile:
            ...
            policy["power_profile_preview"] = lines[:5]

        if perf == "auto":
            policy["warnings"].append("dynamic clocks / possible frametime variance")

        if perf == "manual":
            policy["warnings"].append("manual tuning active")

        if perf == "high":
            policy["warnings"].append("high performance mode active")

        out["cards"].append(policy)

    out["cards_count"] = len(out["cards"])

    return out

async def gpu_policy_wagon(*args, **kwargs):
    try:
        return _egb_gpu_policy_report()
    except Exception as e:
        return {
            "ok": False,
            "source": "gpu-policy-wagon",
...
try:
    Plugin.gpu_policy_wagon = staticmethod(gpu_policy_wagon)
except:
    pass



# GPU_PROFILE_WAGON_90501A

def _egb_gpu_profile_report():
    actions = _egb_gpu_actions_report()

    current = actions.get("current")
        ...
        "profiles": [
            {
                "id": "balanced",
                "label": "Balanced",
                "description": "Auto clocks / lower idle power",
                "safe": True
            },
            {
                "id": "performance",
                "label": "Performance",
                "description": "Force HIGH DPM policy",
                "safe": True
            },
            {
                "id": "stable_frametime",
                "label": "Stable Frametime",
                "description": "Experimental high stability profile",
                "safe": True,
                "experimental": True
            },
            {
                ...
                "description": "Read-only GPU diagnostic profile",
                "safe": True,
                "readonly": True
            }
        ]
...
async def gpu_profile_wagon(*args, **kwargs):
    try:
        return _egb_gpu_profile_report()
    except Exception as e:
        return {
            "ok": False,
            "source": "gpu-profile-wagon",
...
async def gpu_apply_profile(*args, **kwargs):
    try:
        profile = None

        if args and isinstance(args[0], dict):
            profile = args[0].get("profile")
        ...
        if profile == "balanced":
            result = await gpu_set_auto()

        elif profile == "performance":
            result = await gpu_set_high()

        elif profile == "stable_frametime":
            result = await gpu_set_high()

        elif profile == "diagnostic":
            result = _egb_gpu_policy_report()
        ...
        report = _egb_gpu_profile_report()
        ...
        return {
            ...
            "result": result,
            "label": f"profile applied: {profile}"
        }
...
try:
    Plugin.gpu_profile_wagon = staticmethod(gpu_profile_wagon)
    Plugin.gpu_apply_profile = staticmethod(gpu_apply_profile)
except Exception:
    pass
